...
/***************** List View *********************/
// Patrick Maniraho (patrick.maniraho@gmail.com) : adding a list view

//add new view and its button
fcViews.list = AgendaListView;
defaults.defaultButtonText.list = 'list';
defaults.titleFormat.list = 'MMMM YYYY';
defaults.columnFormat.list = 'MMMM D, YYYY';
defaults.timeFormat.list = 'MMMM D';
...
$.extend(ListView.prototype, {
    ...
    renderHtml : function() {
        return '<div class="fc-agendaList"></div>';
    },
    setHeight: function(totalHeight, isAuto) {
    /*
		var eventLimit;
	...
	/* 
	    This is a list of events, we will leave the implementation of 
	    renderDrag, destroyDrag, renderSelection, destroySelection 
	    updateSize, ( setHieght : will think about it, we may choose to set the height or scroll )
	*/
    renderEvents: function(events) {
            var t = this;
			...
			var disDay, disDate, lurl, ltitle;
            ...
            // scan through fake list of events and display them, but check only those of current month
            for (i in displayeventlist) {
                evMonth = formatDate(displayeventlist[i].displayDay, 'MM');
                evYear  = formatDate(displayeventlist[i].displayDay, 'YYYY');
                endDate = null;
                ...
                if ( evMonth == visMonth && evYear == visYear) {
	                ...
	                startDate       = moment(displayeventlist[i].start).format('lll');
	                if ( displayeventlist[i].end ) {
	                    endDate       = moment(displayeventlist[i].end).format('lll');
	                }
	                lurl        = htmlEscape(displayeventlist[i].url);
	                ...
	                if (disDate != temp) {
			            var innerlist = $("<ul class='fc-agendaList'></ul>").appendTo(html);	                	
	                ...
	                // otherwise do the opposite
	                    eventdisplay = $("<li class='fc-agendaList-item'>"+
	                                        "<"+ ((lurl !== 'undefined')  ? "a href='"+ (lurl) +"'" : "div") + 
	                                        " class='fc-agendaList-event fc-eventlist "+ classes.join(' ')  +"'>"+
	                                        "<div class='fc-event-time'>"+
	                                           ( endDate ? (
	                                            "<span class='fc-event-start-time'>"+ htmlEscape(moment(displayeventlist[i].start).format('ll')) +"</span> "+
	                                            "<span class='fc-event-end-time'>"+ ( endDate ? htmlEscape(moment(displayeventlist[i].end).format('ll')) : "") +"</span>" ) 
	                                            : ("<span class='fc-event-all-day'>"+ this.opt('allDayText') +"</span>") ) +
	                                        "</div>"+
	                    ...
	                    eventdisplay = $("<li class='fc-agendaList-item'>"+
	                                    ...
	                                    " class='fc-agendaList-event fc-eventlist "+ classes.join(' ') +"'>"+
	                                        "<div class='fc-event-time'>"+
	                                           ( endDate ? (
	                                            "<span class='fc-event-start-time'>"+ htmlEscape(moment(displayeventlist[i].start).format(fmt)) +"</span> "+
	                                            "<span class='fc-event-end-time'>"+ ( endDate ? htmlEscape(moment(displayeventlist[i].end).format(fmt)) : "") +"</span>" ) 
	                                            :  ("<span class='fc-event-start-time'>"+ htmlEscape(moment(displayeventlist[i].start).format(fmt)) +"</span> ") ) +
	                                        "</div>"+
	                                        "<div class='fc-agendaList-eventDetails'>"+
	                                          "<div class='fc-eventlist-title'>"+ ltitle +"</div>"+
