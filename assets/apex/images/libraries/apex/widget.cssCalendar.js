...
widget.cssCalendar = function( pRegionId, pOptions, pLocale ) {
    ...
    function _init() {
		...
		// We would like the Days heading on month view to be full name rather than short names
		// leave the week and day views to pick their format from locale files
        gOptions.columnFormat = {
                                    month: 'dddd'    // Mon to Monday
                                };
    ...
    function _getEvents( pStart, pEnd, pTimezone, pCallback) {
        ...
        server.plugin( gOptions.ajaxIdentifier,
            {
                x01:       "GET", /* action */
                x02:       pStart.format('YYYYMMDDHHmmss').toString(), //_convertDateToString( pStart.toDate() ), /* start date */
                x03:       pEnd.format('YYYYMMDDHHmmss').toString(), //_convertDateToString( pEnd.toDate() ),   /* end date */
                pageItems: gOptions.pageItems
            }, {
                refreshObject: gRegion$,
                success:       function( pData ) {
...
//            $('.fc-apex-tooltip').css("position", "fixed");
    ...
    widget.cssCalendar.download = function(pFormat) {
        ...
        var lUrl = server.pluginUrl( gOptions.ajaxIdentifier,
            {
                x01:       "DOWNLOAD",
                x02:       view.start.format('YYYYMMDDHHmmss').toString(), //use moment instead of _convertDateToString( view.start.toDate()),
                x03:       view.end.format('YYYYMMDDHHmmss').toString(), //_use moment instead of convertDateToString( view.end.toDate()),              
                x06:       view.name,
                x07:       view.title,
                x10:       pFormat,
                pageItems: gOptions.pageItems
        ...
        if (pType === 'S') {
            ...
            */
            ...
            //lUrl = server.pluginUrl( gOptions.ajaxIdentifier,
                ...
                {
                    x01:       "SEND",
                    x02:       view.start.format('YYYYMMDDHHmmss').toString(), //use moment format instead of _convertDateToString( view.start.toDate()),
                    x03:       view.end.format('YYYYMMDDHHmmss').toString(), // use moment format instead of _convertDateToString( view.end.toDate()),              
                    x06:       view.name,
                    x07:       view.title,
                    x09:       pType,
                    pageItems: gOptions.pageItems
