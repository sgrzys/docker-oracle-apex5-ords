...
(function( widget, $ ) {
    ...
    widget.regionDisplaySelector = function (pRegionDisplaySelectorRegion, pOptions) {
        ...
        var activeTab = null; // NOT GUARANTEED TO BE NOT NULL-- if an RDS has two tabs or less it will not activate and will just hide the header bar!
        var firstVisibleTab = null;  // NOT GUARANTEED TO BE NOT NULL-- if an RDS has two tabs or less it will not activate and will just hide the header bar!
        var tabsContainer$;
        var rtlMode = false;
...
//                    .attr("aria-labelledby", "id of tab");
                ...
                var forceActive = function ( scrollToActive ) {
                    ...
                    if ( pOptions.mode === "standard" ) {
                        if(jQuery().stickyWidget) {
                            activeTab.el$
                        ...
                        if (jQuery().fullCalendar) {
                            activeTab.el$
                                .find(".fc") // invoking full calendar events on non-full calendar instances
                                             // does not throw an exception, so this is safe.
                                .fullCalendar("rerenderEvents");
                        }
                    }
                    tabsContainer$.trigger("rdsregionchange", [activeTab, pOptions.mode]);
...
//                            slider$ = null;
            ...
            if ( rtlMode ) {
                var tempTab = lastVisibleTab;
                lastVisibleTab = firstVisibleTab;
                firstVisibleTab = tempTab;
            }
            if (activeTab == null) {
        ...
        var getNextActiveTabFromSiblings = function( tab ) {
            ...
            var next = tab.getNextVisible( rtlMode );
            if (next != null) {
                if (next.getTop() < currentScrollPosition) {
                    return next;
                }
            }
            var previous = tab.getPreviousVisible( rtlMode );
            if (previous != null) {
        ...
        var initializeStandardTabs = function () {
             ...
             */
            if ( activeTab && activeTab.href != SHOW_ALL ) {
