...
begin
    if '^UPGRADE' = '2' then
        ...
        wwv_flow_upgrade.log('disable foreign key constraints in CDB: owner='||'^APPUN');
        for t in 1..wwv_flow_upgrade.g_tabs.count loop
            wwv_flow_upgrade.disable_constraints(p_owner_name=>'^APPUN',p_object_name=>wwv_flow_upgrade.g_tabs(t));
        end loop;
        wwv_flow_upgrade.log('disable triggers in CDB: owner='||'^APPUN');
        for t in 1..wwv_flow_upgrade.g_tabs.count loop
            wwv_flow_upgrade.disable_triggers(p_owner_to=>'^APPUN', p_table_name=>wwv_flow_upgrade.g_tabs(t));
        end loop;
    end if;
end;
/
